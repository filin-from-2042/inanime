		...
		while ($arCity = $rsLocationsList->GetNext())
		{
            ...
            {/*
                $res = \Bitrix\Sale\Location\LocationTable::getList(array(
                    'filter' => array('=ID' => $arParams['LOCATION_VALUE'],'=NAME.LANGUAGE_ID' => LANGUAGE_ID),
                    'select' => array('*','ID','NAME_RU' => 'NAME.NAME','NAME.*')
                ));

                while($item = $res->fetch())
                {
//                    $locationString = (strlen($item["NAME_RU"]) > 0 ? $item["NAME_RU"].", " : "").$locationString;
                    $arResult["CITY_LIST"][] = array(
                        "ID" => $item['ID'],
                        "CITY_ID" => $item['ID'],

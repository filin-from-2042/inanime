...
function JsSuggestSale(oHandler, sParams, sParser, domain, ssubmit)
{
	...
	t.Send = function(sSearch)
	{
		...
		url = '/local/components/inanime/sale.ajax.locations/search.php';
		if(t.domain)
			url = domain + '/local/components/inanime/sale.ajax.locations/search.php';
	...
	t.Print = function(aArr)
	{
		...
		for (tmp_ in aArr)
		{
			...
			aRes['NAME'] = TCJsUtils.htmlspecialcharsEx(locName);
                        aRes['SHORT_NAME'] = TCJsUtils.htmlspecialcharsEx(aEl['NAME']);
	...
	t.Replace = function()
	{
		...
		if (typeof t.oActive == 'string')
		{
			...
			if (typeof tmp == 'object')
			{
				...
				elEntities.innerHTML = TCJsUtils.htmlspecialcharsback(tmp['SHORT_NAME']);

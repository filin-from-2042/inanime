...
if ($arParams["AJAX_CALL"] != "Y"
	...
	foreach ($arParams["LOC_DEFAULT"] as $val):
		...
		<div class="input-container">
            <input onChange="<?=$arParams["ONCITYCHANGE"]?>;" <?=$checked?> type="radio" name="NEW_LOCATION_<?=$arParams["ORDER_PROPS_ID"]?>" value="<?=$val["ID"]?>" id="loc_<?=$val["ID"]?>" class="form-control"/>
            <label for="loc_<?=$val["ID"]?>"><?=$val["LOC_DEFAULT_NAME"]?></label></div>
	<?endforeach;?>
	<div class="input-container">
        <input <? if($isChecked!="Y") echo 'checked';?> type="radio" onclick="clearLocInput();" name="NEW_LOCATION_<?=$arParams["ORDER_PROPS_ID"]?>" value="0" id="loc_0" class="form-control" />
...
<input
	...
	class="form__text form-control location-input" type="text"
	autocomplete="off"

...
if ($_SERVER["REQUEST_METHOD"] == "POST" && ($_REQUEST["save"] <> '' || $_REQUEST["apply"] <> '') && check_bitrix_sessid()) {
   ...
   if(CModule::IncludeModule("subscribe"))
   {
    ...
    if (COption::GetOptionString('main', 'use_encrypted_auth', 'N') == 'Y') {
        ...
        if (($arKeys = $sec->LoadKeys())) {
            ...
            if ($errno == CRsaSecurity::ERROR_SESS_CHECK)
                $strError .= GetMessage("main_profile_sess_expired") . '';
            elseif ($errno < 0)
                $strError .= GetMessage("main_profile_decode_err", array("#ERRCODE#" => $errno)) . '';
        }
    }
    ...
    if ($strError == '') {
        ...
        if ($_REQUEST["change-password"] == "Y") {
            $rsUser = CUser::GetByID($USER->GetId());
            $arUser = $rsUser->Fetch();
            $arFields["LOGIN"] = $arUser["LOGIN"];
            if (strlen($_REQUEST["OLD_PASSWORD"]) == 0)
                $strError.='Не указан текущий пароль<br />'; else if (!isUserPassword($USER->GetId(), $_REQUEST["OLD_PASSWORD"]))
                $strError.='Указан неверный пароль<br />';
        ...
        $freeCardExists = false;
        if ($USER->GetID() > 0) {
            $USER_FIELD_MANAGER->EditFormAddFields("USER", $arFields);
            $card = $arFields["UF_USER_CARD"];
            $card_confirm = ($_REQUEST["CARD_CONFIRM"])?$_REQUEST["CARD_CONFIRM"]:false;

            //write
            $userHasThisCard = (boolean)CIBlockElement::GetList(array(), array("IBLOCK_CODE" => "USER_DISCOUNT", "PROPERTY_CARD_CODE" => $card, "PROPERTY_DISCOUNT_USER" => $USER->GetID()), array());
                //clear

	    $dbDiscountCards = CIBlockElement::GetList(array(), array("IBLOCK_CODE" => "USER_DISCOUNT", "PROPERTY_CARD_CODE" => $card, "PROPERTY_CARD_CODE_CONFIRM" => $card_confirm, "PROPERTY_DISCOUNT_USER" => false), false, false, array("NAME", "ID", "PROPERTY_CARD_CODE", "PROPERTY_CARD_CODE_CONFIRM", "PROPERTY_DISCOUNT_USER", "PROPERTY_DISCOUNT_GROUP"));
            if ($arDiscountCards = $dbDiscountCards->fetch()) {
                $freeCardExists = true;
		$arDiscountCards_ID=$arDiscountCards["ID"];
            }
        }
		
        if (!$freeCardExists && !$userHasThisCard) {
            $arFields["UF_USER_CARD"] = null;
            if (strlen($_REQUEST["UF_USER_CARD"])>0 || strlen($_REQUEST["UF_USER_CARD_CODE"])>0)$strError .="Указан недействительный номер карты";
        }
    ...
    if ($strError == '') {
        ...
        $dbDiscountCards2 = CIBlockElement::GetList(array(), array("IBLOCK_CODE" => "USER_DISCOUNT", "PROPERTY_DISCOUNT_USER" => $USER->GetID()), false, false, array("NAME", "ID", "PROPERTY_CARD_CODE", "PROPERTY_DISCOUNT_USER", "PROPERTY_DISCOUNT_GROUP"));
        while ($arDiscountCards2 = $dbDiscountCards2->fetch()) {

            CIBlockElement::SetPropertyValues($arDiscountCards2["ID"], 5, false, "DISCOUNT_USER");
        }

	 if ((int)$arDiscountCards_ID>0) 
               CIBlockElement::SetPropertyValues($arDiscountCards_ID, 5, $USER->GetID(), "DISCOUNT_USER");

...
{
    $arCheckBasketItems[] = $arItems;
}
if(count($arCheckBasketItems)<=0)  LocalRedirect('/');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid())) {
        ...
        elseif ($_POST["do_register"] == "Y" && $arResult["AUTH"]["new_user_registration"] == "Y") {
            if (strlen($_POST["NEW_NAME"]) <= 0)
                $arResult["ERROR"]["NEW_NAME"] = GetMessage("STOF_ERROR_REG_NAME");

            if (strlen($_POST["NEW_LAST_NAME"]) <= 0)
                $arResult["ERROR"]["NEW_LAST_NAME"] = GetMessage("STOF_ERROR_REG_LASTNAME");

            if (strlen($_POST["NEW_EMAIL"]) <= 0)
                $arResult["ERROR"]["NEW_EMAIL"] = GetMessage("STOF_ERROR_REG_EMAIL");
            elseif (!check_email($_POST["NEW_EMAIL"]))
                $arResult["ERROR"]["NEW_EMAIL"] = GetMessage("STOF_ERROR_REG_BAD_EMAIL");

            $arResult["AUTH"]["NEW_EMAIL"] = $_POST["NEW_EMAIL"];
            
            if ($_POST["ACCEPT_NEW"] != "Y")
            $arResult["ERROR"]["ACCEPT_NEW"] = "Вы должны быть ознакомлены с условиями обработки персональных данных и с политикой конфиденциальности компании";
            ...
            if (empty($arResult["ERROR"])) {
                ...
                if ($_POST["NEW_GENERATE"] == "Y") {
                    ...
                    if (strlen($_POST["NEW_LOGIN"]) <= 0)
                        $arResult["ERROR"]["NEW_LOGIN"] = GetMessage("STOF_ERROR_REG_FLAG");

                    if (strlen($_POST["NEW_PASSWORD"]) <= 0)
                        $arResult["ERROR"]["NEW_PASSWORD"] = GetMessage("STOF_ERROR_REG_FLAG1");
                    if (strlen($_POST["NEW_PASSWORD_CONFIRM"]) <= 0)
                        $arResult["ERROR"]["NEW_PASSWORD_CONFIRM"] = GetMessage("STOF_ERROR_REG_FLAG1");
                    ...
                    if (strlen($_POST["NEW_PASSWORD"]) > 0 && strlen($_POST["NEW_PASSWORD_CONFIRM"]) > 0 && $_POST["NEW_PASSWORD"] != $_POST["NEW_PASSWORD_CONFIRM"]) {
                        $arResult["ERROR"]["NEW_PASSWORD"] = GetMessage("STOF_ERROR_REG_PASS");
                        $arResult["ERROR"]["NEW_PASSWORD_CONFIRM"] = GetMessage("STOF_ERROR_REG_PASS");
                    }
            ...
            $arResult["ERROR"]["ACCEPT_NEW"] = "Вы должны быть ознакомлены с условиями обработки персональных данных и с политикой конфиденциальности компании";
                    ...
                    $arResult["AUTH"]["NEW_SECOND_NAME"] = $_POST["NEW_SECOND_NAME"];
            ...
            if (empty($arResult["ERROR"])) {
/*if (!$USER->IsAuthorized() && $_REQUEST["ACCEPT_NEW"] != "Y")
            $arResult["ERROR"]["ACCEPT_NEW"] = "Вы должны быть ознакомлены с условиями обработки персональных данных и с политикой конфиденциальности компании";*/
                ...
                if ($arAuthResult != False && $arAuthResult["TYPE"] == "ERROR")
                    $arResult["ERROR"]["CAPTCHA_ERROR"] = GetMessage("STOF_ERROR_REG") . ((strlen($arAuthResult["MESSAGE"]) > 0) ? ": " . $arAuthResult["MESSAGE"] : "" );
                else {
                    ...
                    if ($USER->IsAuthorized()) {
                        ...
                        if ($arAuthResult["TYPE"] == "OK") {
                            ...
                            $fields = Array(
                                ...
                                "PERSONAL_PHONE" => $_POST["NEW_PHONE"]
                            );
            ...
            $arResult["AUTH"]["~NEW_SECOND_NAME"] = $_POST["NEW_SECOND_NAME"];
            $arResult["AUTH"]["NEW_SECOND_NAME"] = htmlspecialcharsEx($_POST["NEW_SECOND_NAME"]);
            $arResult["AUTH"]["~NEW_PHONE"] = $_POST["NEW_PHONE"];
            $arResult["AUTH"]["NEW_PHONE"] = htmlspecialcharsEx($_POST["NEW_PHONE"]);
            $arResult["AUTH"]["~NEW_LAST_NAME"] = $_POST["NEW_LAST_NAME"];
    ...
    if (strlen($_REQUEST["ORDER_ID"]) <= 0) {
        ...
        while ($arItem = $dbBasketItems->GetNext()) {
            if ($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y") {
                ...
                $arItem["SET_PARENT_ID"] = intval($arItem["SET_PARENT_ID"]);
                $arItem["TYPE"] = intval($arItem["TYPE"]);
                $arItem["CALLBACK_FUNC"] = trim($arItem["CALLBACK_FUNC"]);
                $arItem["PRODUCT_PROVIDER_CLASS"] = trim($arItem["PRODUCT_PROVIDER_CLASS"]);
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["confirmorder"]) && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid())) {
            ...
            if($_POST["removeCoupon"])  DiscountCouponsManager::delete($_POST["removeCoupon"]);

            if(isset($_POST['addresProfile']))
            {
                // установка активного профиля
                $newUser = new CUser;
                $fields = Array(
                    "UF_CUSTOMER_PROFILE" => intval($_POST['addresProfile']),
                );
            ...
            while ($arOrderProps = $dbOrderProps->GetNext()) {
                ...
                if ($bErrorField)
                    $arResult["ERROR"][$arOrderProps["CODE"]] = GetMessage("SOA_ERROR_REQUIRE") . " \"" . $arOrderProps["NAME"] . "\"";
        ...
        if ($arUser = $rsUser->Fetch()) {
            if ($arUser["UF_USER_TYPE"] == USER_TYPE_PERSONAL)
                $currentType = 1; elseif ($arUser["UF_USER_TYPE"] == USER_TYPE_COMPANY)
                $currentType = 2;
        }
        ...
        while ($arPersonType = $dbPersonType->GetNext()) {
            ...
            if ($currentType == $arPersonType["ID"]/* || IntVal($arUserResult["PERSON_TYPE_ID"]) <= 0 */) {
                $arUserResult["PERSON_TYPE_ID"] = $arPersonType["ID"];
                $arPersonType["CHECKED"] = "Y";
                if ($_REQUEST["PERSON_TYPE"] != $arPersonType["ID"])
                    $arPersonType["NOT_CONFIRMED"] = "Y";
            }
        ...
        // name для LOCATION_ID
        $db_props = CSaleOrderProps::GetList(
            array("SORT" => "ASC"), array(
                "PERSON_TYPE_ID" => $arUserResult["PERSON_TYPE_ID"], "CODE" => "LOCATION"
            ), false, false, array()
        );
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST")
            if($locationProp && isset($_POST[$locationProp['FIELD_NAME']])) $arResult['TEMPORARY_FIELDS_DATA'][$locationProp['FIELD_NAME']] = $_POST[$locationProp['FIELD_NAME']];
        ...
        if($USER->IsAuthorized())
        {
            ...
            $arResult['USER_HELPFULL_VALUES']['CURRENT_PROFILE_ID'] = $currentProfileID;
            ...
            {
                /*
                 * массив вида 'profile_id'=>array('PROPS'=>array(
                 *                                                  'LOCATION'=>1125,
                 *                                                  'ADDRESS'=>'...',
                 *                                                 ......
                 *                                                  )
                ...
                while ($profile = $profiles->Fetch())
                {
                    ...
                    // текстовые распарсенные данные по профилю для js
                    $addressData = array();
                    ...
                    $addressData = $arResult['USER_PROFILES'][$profile['ID']]['parsedAddressText'] = $resultArray;

                    // текстовые данные по текущему местонахождению
                    $locationTextData=CSaleLocation::GetByID($arResult['USER_PROFILES'][$profile['ID']]['PROPS']['LOCATION']);
                    ...
                    $addressData['locationID'] = $arResult['USER_PROFILES'][$profile['ID']]['PROPS']['LOCATION'];

                    $arResult['arJSParams']['saleProfiles'][$profile['ID']] = $addressData;
                }
            }
        }

        // Для доставки
        // Установка location_id для авторизованного из профиля и для неавторизованного из определенного местонахождения по умолчанию
        // При аяксе запросе выставляем переданное значение в посте
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            $arUserResult["DELIVERY_LOCATION"] = $arResult['TEMPORARY_FIELDS_DATA'][$locationProp['FIELD_NAME']];
        }else{
            if($USER->IsAuthorized())
            {
        ...
        if (IntVal($arUserResult["DELIVERY_LOCATION"]) > 0) {
            ...
            while ($arDelivery = $dbDelivery->Fetch()) {
                ...
                if (IntVal($arUserResult["DELIVERY_ID"]) == IntVal($arDelivery["ID"])) {
                    ...
                    $arResult["D_ID"][] = array($arUserResult["DELIVERY_ID"], IntVal($arDelivery["ID"]), $arResult["DELIVERY_PRICE"]);
                }
            }
            if (!$bFound && !empty($arUserResult["DELIVERY_ID"]) && strpos($arUserResult["DELIVERY_ID"], ":") === false) {
                $arUserResult["DELIVERY_ID"] = "";
        ...
        foreach ($arResult["ORDER_PROP"]["USER_PROPS_Y"] as $prop_key => $prop)
        ...
        if ($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"])) {
            ...
            if ($USER->IsAuthorized() && empty($arResult["ERROR"])) {
                ...
                $withdrawSum = 0.0;
                if (empty($arResult["ERROR"])) {
                    if ($arResult["PAY_FROM_ACCOUNT"] == "Y" && $arUserResult["PAY_CURRENT_ACCOUNT"] == "Y" && (($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && DoubleVal($arResult["USER_ACCOUNT"]["CURRENT_BUDGET"]) >= DoubleVal($totalOrderPrice)) || $arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] != "Y")) {
                        $withdrawSum = CSaleUserAccount::Withdraw(
                                        $USER->GetID(), $totalOrderPrice, $arResult["BASE_LANG_CURRENCY"], $arResult["ORDER_ID"]
                        );
                ...
                if (empty($arResult["ERROR"])) {
                    ...
                    $arBasketList = array();
                    $dbBasketItems = CSaleBasket::GetList(
                                    array("NAME" => "ASC"), array("ORDER_ID" => $arResult["ORDER_ID"]), false, false, array("ID", "PRODUCT_ID", "NAME", "QUANTITY", "PRICE", "CURRENCY", "TYPE", "SET_PARENT_ID")
                    );
                    while ($arItem = $dbBasketItems->Fetch()) {
                        if (CSaleBasketHelper::isSetItem($arItem))
                            continue;
